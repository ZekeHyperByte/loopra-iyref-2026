...
import { ArrowDownToLine, FlaskConical, Flame, Loader2, Package, TrendingUp, TrendingDown } from "lucide-react";
import { Button } from "@/components/ui/button";
import { motion } from "framer-motion";
import { useWasteDeposits } from "@/hooks/useWasteDeposits";
import {
  aggregateHubs,
...
const Inventory = () => {
  const { rows, loading, error, refetch } = useWasteDeposits();
  ...
  return (
    <div className="p-6 space-y-6 relative min-h-[480px]">
      <PageHeader
        ...
        description="Live crude ethanol (Grade A) and biogas potential (Grade B) from waste_deposits, synced in real time."
      >
      ...
      <div className={cn("space-y-6", loading && "pointer-events-none opacity-50")}>
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <StatCard
            Icon={TrendingUp}
            label="Avg throughput (samples)"
            value={totals.avgThroughputTph != null ? Math.round(totals.avgThroughputTph * 10) / 10 : 0}
            suffix=" t/h"
            decimals={1}
          />
        </div>
      ...
      {loading && (
        <div className="absolute inset-0 z-20 flex flex-col items-center justify-center gap-3 bg-background/80 backdrop-blur-sm">
          ...
          <p className="text-sm font-mono text-muted-foreground tracking-wide">Loading inventory from Supabase…</p>
        </div>
      )}
    </div>
  );
